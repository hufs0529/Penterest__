...
def inference(gif, model_name):
  ...
  obj = s3_client.get_object(Bucket="penterest", Key=f"gifs/{gif}".replace("mp4","gif"))
  file_stream = obj['Body']
  img = Image.open(file_stream)

  use_beam_search = False 
  ImageForScikit = np.array(img)
  
  pil_image = PIL.Image.fromarray(ImageForScikit)  
